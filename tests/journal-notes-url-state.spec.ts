...
test.describe("Journal/Notes composer view is reflected in the URL", () => {
  ...
  test("direct load of /journal?new=1 lands on the composer (not just client nav)", async ({ page }) => {
    await page.goto("/journal?new=1");
    await page.waitForLoadState("networkidle").catch(() => {});
    await page.waitForTimeout(800);
    await expect(page.getByRole("heading", { name: /Record, type or paste a new entry/i })).toBeVisible();
    expect(new URL(page.url()).searchParams.get("new")).toBe("1");
  });

  test("direct load of /notes?new=1 lands on the composer (not just client nav)", async ({ page }) => {
  ...
  test("in-page Journal '+ New entry' button updates the URL, and back/forward works", async ({ page }) => {
  ...
  test("in-page Notes '+ New note' button updates the URL, and back/forward works", async ({ page }) => {
  ...
  test("clicking a Journal history session exits the composer and clears ?new=1", async ({ page }) => {
    await page.route("**/rest/v1/organiser_sessions**", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify([
          { id: "sess-1", user_id: "00000000-0000-4000-8000-000000000001", source: "journal", created_at: "2026-01-01T00:00:00Z" },
        ]),
      }),
    );
