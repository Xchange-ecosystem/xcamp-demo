...
const MOCK_TASK = {
  ...
  body_html: "<p>seeded</p>",
  body_markdown: "seeded",
  tags: [],
  detail: {},
...
const MOCK_NOTE = {
  id: NOTE_ID,
  title: "Vymo WBS kickoff note",
  note_type: "note",
  done: false,
  body_html: "<p>seeded note</p>",
  body_markdown: "seeded note",
  tags: [],
  detail: {},
  owner_central_id: CENTRAL_USER_ID,
  tenant_id: TENANT_ID,
...
async function setupRoutes(page: Page) {
  ...
  await page.route(`**/${SUPABASE_PROJECT_REF}.supabase.co/rest/v1/projects**`, (r) => {
    ...
    const row = {
      id: PROJECT_ID,
      title: "Vymo Project WBS Generation",
      description: "seeded",
      tags: [],
    };
    r.fulfill({
  ...
  await page.route(`**/${SUPABASE_PROJECT_REF}.supabase.co/rest/v1/notes**`, (r) => {
    ...
    if (url.includes(TASK_ID)) {
      ...
      r.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify([MOCK_TASK, MOCK_NOTE]),
      });
    }
  });
}
...
test.describe("Investor/Founder dashboard split + Goals input + My Notes filter", () => {
  test("founder sees Overview + Dashboard tabs and can switch", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/investor-founder/founder-dashboard-tab.png",
      fullPage: true,
    });
  });

  test("investor sees Dashboard only — no tab switcher, no Overview reachable", async ({
    page,
  }) => {
    await setupRoutes(page);
    ...
    await page.screenshot({
      path: "test-results/investor-founder/investor-dashboard-only.png",
      fullPage: true,
    });
  ...
  test("founder Project Details nav opens the tabbed project page, not the flat page", async ({
    page,
  }) => {
    await setupRoutes(page);
    await injectSession(page, "founder");
    ...
    await expect(page.getByRole("button", { name: "Dashboard" })).toBeVisible();
    await page.screenshot({
      path: "test-results/investor-founder/project-details-nav.png",
      fullPage: true,
    });
  });
  ...
  test("Goals page create-goal field is a multiline, ~4-row textarea", async ({ page }) => {
    ...
    expect(rows).toBe("4");
    await page.screenshot({
      path: "test-results/investor-founder/goals-textarea.png",
      fullPage: true,
    });
  });

  test("My Notes: project filter finds a note linked only via objective_notes", async ({
    page,
  }) => {
    await setupRoutes(page);
    await injectSession(page, "founder");
    ...
    await expect(page.getByText(`Project: Vymo Project WBS`, { exact: false })).toBeVisible({
      timeout: 15_000,
    });
    ...
    await page.screenshot({
      path: "test-results/investor-founder/my-notes-filter-fixed.png",
      fullPage: true,
    });
  });
});
