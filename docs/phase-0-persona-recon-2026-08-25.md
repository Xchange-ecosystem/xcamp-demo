...
PR #85 built the rail on 2026-08-08. The very next day, PR #94 flipped the _entire_ new
...
rail's own "Detail" tab happens to drive the _same_ `useSidepanel()` context, so clicking it
...
_"This used to be a parallel implementation of AppShell... now AppShell's transparent variant"_
...
2. There is _also_ a dedicated, always-reachable post-login control at **`/profile/appearance`**
...
_only_ — every "theme" control found is a light/dark/system picker whose dark state happens to be
reskinned as "Nox."
...
- "Here's what I recommend" tool-card set: `:1293-1312` — same hardcoded 3-card JSX block
  described above (this view _is_ what renders the overlay's sibling content once dismissed; the
...
Separately, and not mentioned in the brief at all: there's a _second_, orthogonal tab dimension,
...
- `src/lib/journal-api.ts:138-160` exports an `answerWithContext()` function that POSTs to
  ...
  the _same_ call, differentiated only by whether `project_id` is set:
  ```ts
...
- **Which lane actually serves this today — confirmed via live network trace + source, not code
  ...
  design — its `/api/search` and `/api/generate` routes are commented _"called server-to-server
  from xcamp-backend"_ / _"S2S trusted path; no caller JWT"_ (`chi-orchestration/src/server.ts:67-91`).
...
Two `jsonb` columns exist that _could_ carry an ad-hoc persona value without a migration
...
on this data: only 3 of the 10 owned projects have _any_ `object_memberships` row at all (even an
