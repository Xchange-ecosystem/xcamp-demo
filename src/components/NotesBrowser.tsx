...
export function NotesBrowser({
  ...
  const linked = useMemo(() => linkedQuery.data ?? new Set<string>(), [linkedQuery.data]);
  ...
  const noteProjects = useMemo(
    () => noteProjectsQuery.data ?? new Map<string, Set<string>>(),
    [noteProjectsQuery.data],
  );
  ...
  const visibleNotes = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      return (
        (new Date(b[key] || b.created_at).getTime() - new Date(a[key] || a.created_at).getTime()) *
        dir
      );
    });
    return list;
  ...
  const bulkAssignMut = useMutation({
    mutationFn: (input: { ids: Set<string>; projectId: string | null }) =>
      bulkAssignProject(
        user!,
        notes.filter((n) => input.ids.has(n.id)),
        input.projectId,
      ),
    onSuccess: () => {
      invalidate();
          ...
          {effCollapsed ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <button
                ...
                onClick={() => {
                  onRequestNew?.();
                  setEditing({ mode: "new", initialProjectId: activeProjectId ?? undefined });
                  setCollapsed(false);
                }}
              >
                <Plus size={16} />
            ...
            <>
                      ...
                      {(Object.keys(SORT_LABELS) as SortKey[]).map((k) => (
                        <button
                          ...
                          style={{
                            ...
                            border: "none",
                            cursor: "pointer",
                            background: sort === k ? "var(--skin-surface2)" : "transparent",
                            color: "var(--skin-ink)",
                          }}
                        >
                          {SORT_LABELS[k]}
                          {sort === k && (
                            <Check size={14} style={{ color: "var(--skin-accent)" }} />
                          )}
                        </button>
                      ))}
                      <div
                        style={{ height: 1, background: "var(--skin-line)", margin: "5px 4px" }}
                      />
                      <button
              ...
              {activeFilterCount > 0 && (
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  {filterProject && (
                    <FilterChip
                      label={`Project: ${projectName(filterProject) ?? "Unknown"}`}
                  ...
                  {typeFilterActive && (
                    <FilterChip
                      label={`Type: ${
                        allNoteTypes
                          .filter((t) => filterTypes.includes(t))
                          .map(noteTypeLabel)
                          .join(", ") || "None"
                      }`}
                  ...
                  {filterLinked && (
                    <FilterChip label="Linked only" onClear={() => setFilterLinked(false)} />
                  )}
                  <button
                    onClick={clearAllFilters}
                    style={{
              ...
              {selectMode && selected.size > 0 && (
                <div
                  className="mb-2 flex flex-wrap items-center gap-2 rounded-md p-2"
                  style={{ background: "var(--skin-surface2)" }}
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm(`Delete ${selected.size} note(s)?`))
                        bulkArchiveMut.mutate(selected);
                    }}
              ...
              >
                {notesQuery.isLoading && (
                  <p style={{ color: "var(--skin-ink-faint)", fontSize: 13 }}>Loading notes…</p>
                )}
                ...
                {visibleNotes.map((note) => {
                  ...
                  return (
                    <div
                      ...
                      data-active={active}
                      onClick={() =>
                        selectMode ? toggleSelect(note.id) : setEditing({ mode: "edit", note })
                      }
                    >
                      <div className="flex items-start gap-2">
                        {selectMode && (
                          <span
                            style={{
                              color: isSel ? "var(--skin-accent)" : "var(--skin-ink-faint)",
                              marginTop: 2,
                            }}
                          >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            <div
                              className="x-note-card__title"
                              style={{ fontSize: 14, fontWeight: 600, color: "var(--skin-ink)" }}
                            >
                              {note.title || "Untitled"}
                            </div>
                            <div className="flex flex-shrink-0 items-center gap-1">
                              {linked.has(note.id) && (
                                <span className="x-badge-linked">linked</span>
                              )}
                              <button
                                type="button"
                          ...
                          >
                            <span style={{ fontSize: 12, color: "var(--skin-ink-faint)" }}>
                              ...
                              {projectName(noteProjectId(note)) &&
                                ` · ${projectName(noteProjectId(note))}`}
                            </span>
        ...
        >
          ...
          {editing ? (
            <NoteEditor
              ...
              onArchive={
                editing.mode === "edit" ? () => archiveMut.mutate(editing.note) : undefined
              }
              onOrganise={editing.mode === "edit" ? () => setOrganising(editing.note) : undefined}
              onExpand={
                editing.mode === "edit"
                  ? () =>
                      useFullscreenItemStore
                        .getState()
                        .open(editing.note.id, editing.note.note_type)
                  : undefined
              }
            />
          ) : (
            ...
            >
              <div>
                <p style={{ fontSize: 15, color: "var(--skin-ink-soft)" }}>
