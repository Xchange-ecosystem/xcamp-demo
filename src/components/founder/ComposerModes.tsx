...
// Text is untouched (still owned by demo.founder.index.tsx). Transcript on
// Founder Home now runs TranscriptSimBody below — a fixture-only,
// content-blind simulation (B3) — instead of the real pipeline. The real
// end-to-end pipeline (TranscriptModeBody's single-file drop + extraction
// read, paired with TranscriptOverlay.tsx) stays in this file unused by
// Founder Home for now, reserved for a future /admin/recap session. Voice,
// Upload and Link agent render designed empty states and are visibly inert,
// not broken.
...
/* -------------------------------------------------------------------- */
/* B3 — Founder Home's Transcript tab (fixture-only, content-blind)      */
/* -------------------------------------------------------------------- */
...
function iconForPendingFile(file: PendingFile) {
...
export function TranscriptSimBody({
  text,
  onTextChange,
  files,
  onFilesAdded,
  ...
  return (
    <div
      ...
      onDrop={(e) => {
        e.preventDefault();
        setDragOver(false);
        if (!loading && e.dataTransfer.files.length > 0) onFilesAdded(e.dataTransfer.files);
      }}
    >
      <textarea
        value={text}
        ...
        disabled={loading}
        rows={3}
        placeholder="Paste a transcript, or drop a recording/notes file below — Chi simulates a read for this demo."
      ...
      <button
        ...
        onClick={() => fileInputRef.current?.click()}
        disabled={loading}
        className="flex w-full flex-col items-center gap-1.5 rounded-lg border-[1.5px] border-dashed py-5 text-center transition-colors"
        style={{
          borderColor: dragOver ? "var(--skin-accent)" : "var(--skin-line)",
          background: dragOver
            ? "var(--skin-accent-wash, color-mix(in oklch, var(--skin-accent) 8%, transparent))"
            : "transparent",
          color: "var(--skin-ink-soft, var(--muted-foreground))",
        }}
      >
        <Upload size={20} style={{ color: "var(--skin-ink-faint, var(--muted-foreground))" }} />
        <b className="text-[13px] font-medium text-foreground">
          Drop files here, or click to browse
        </b>
        <small className="text-[12px]">
          Recording, notes, anything — Chi never reads the contents in this demo.
        </small>
      </button>
      ...
      {files.length > 0 && (
        <div className="flex flex-col gap-1.5">
          {files.map((file) => {
            const Icon = iconForPendingFile(file);
            return (
              <div
                key={file.id}
                ...
                style={{ background: "var(--skin-raised, var(--muted))" }}
              >
                <Icon size={16} style={{ color: "var(--skin-accent)" }} />
      ...
      <div className="flex justify-end">
        <Button size="sm" disabled={loading} onClick={onSend}>
          {loading ? (
            <>
              <Loader2 size={14} className="animate-spin" /> Chi is reading…
            </>
          ) : (
            <>
              Send to Chi <Send size={14} />
