...
function defaultTopicEntityType(topic: JournalTopic): EntityType {
  if (topic.organiser_proposals.some((p) => p.proposal_type === "new_objective"))
    return "objective";
      ...
      {types.map(({ value, label }) => (
        <button
          ...
          style={{
            ...
            borderRadius: 999,
            border:
              selected === value ? "1px solid var(--skin-accent)" : "1px solid var(--skin-line)",
            background:
              selected === value
  ...
  switch (entityType) {
    case "objective":
      return {
        ...
        payload: { title, project_id: basePayload.project_id },
      } as unknown as AICard["proposal"];
    case "task":
      return {
        tool: "create_task",
    ...
    case "resource":
      return {
        tool: "add_note",
        payload: { ...basePayload, title, note_type: "reference" },
      } as unknown as AICard["proposal"];
    case "note":
    default:
      return {
        tool: "add_note",
        payload: { ...basePayload, title, note_type: "note" },
      } as unknown as AICard["proposal"];
  }
}
...
function statusColors(status: SessionStatus): { bg: string; fg: string } {
  switch (status) {
    case "committed":
      return {
        bg: "color-mix(in srgb, var(--skin-accent) 18%, transparent)",
        fg: "var(--skin-accent)",
      };
    case "partial":
  ...
  return (
    <button
      ...
      style={{
        ...
        background:
          "radial-gradient(circle at 35% 30%, color-mix(in srgb, var(--skin-accent) 45%, #fff), var(--skin-accent) 65%, color-mix(in srgb, var(--skin-accent) 75%, #001a14))",
        display: "flex",
        alignItems: "center",
...
function NestedTaskCard({
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 14, fontWeight: 700, color: "var(--skin-ink)" }}>
          {topic.title}
        </span>
        <span
          style={{
...
export function JournalFlow({
  ...
  const [savedTopicTargets, setSavedTopicTargets] = useState<Map<string, EntityPanelTarget>>(
    new Map(),
  );
  ...
  const applyTopic = async (
  ...
  ): Promise<EntityPanelTarget | undefined> => {
    if (!user) return;
    if (selectedType === "objective" && !activeProjectId) {
      throw new Error(
        "Select a project to create an objective from this entry. Open any project from the sidebar first.",
      );
    }
    ...
    const allProposalsAreNewObjective = topic.organiser_proposals.every(
      (p) => p.proposal_type === "new_objective",
    );
    const wouldFailWithoutObjective =
      hasProposals && allProposalsAreNewObjective && selectedType !== "objective";
    ...
    if (hasProposals && topic.organiser_session_id && !wouldFailWithoutObjective) {
      ...
      if (panelTarget) {
        ...
        throw new Error(
          detail ??
            "Entry was processed but no entity was created. Check that a project is selected.",
        );
      }
    } else {
      if (hasProposals && topic.organiser_session_id && wouldFailWithoutObjective) {
      ...
      const note = await createNote(user, {
        title: topic.title,
        bodyHtml,
        noteType: noteTypeMap[selectedType] ?? "note",
      });
  ...
  const handleHistoricalObjectiveCommitted = async (
    entityId: string,
                ...
                {historyOpen && (
                  ...
                  >
                    {sessionsQuery.isLoading && (
                      <div
                        style={{ fontSize: 12, color: "var(--skin-ink-faint)", padding: "4px 2px" }}
                      >
                        Loading…
                      </div>
                    )}
                    ...
                    {sessions.map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            style={{ fontSize: 12, color: "var(--skin-ink-soft)", marginTop: 2 }}
                          >
            ...
            {screen === "input" && (
              ...
              >
                <div style={{ textAlign: "center", maxWidth: 460 }}>
                  <h2
                    style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "var(--skin-ink)" }}
                  >
                  ...
                  >
                    Talk about as many topics with as many details as you wish. Chi will understand
                    where topics belong and help you organise it.
                  </p>
                </div>
...
function SessionHistoryView({
  ...
  const originalText: string | null =
    typeof sessionQuery.data?.context?.text === "string" ? sessionQuery.data.context.text : null;
  ...
  const handleHistoricalAccept = async (
    proposalId: string,
    overrideType?: "objective" | "task",
  ) => {
    ...
    try {
      ...
      if (commitResult.results?.length) {
        ...
        const type: EntityPanelTarget["type"] =
          first.proposal_type === "new_objective" ? "objective" : "note";
        openEntity({ type, id: first.id, objectiveId: first.objective_id });
        if (overrideType === "objective" && displayText) {
          const proposalTitle =
            historicalProposals?.find((p) => p.id === proposalId)?.title ?? "Untitled";
  ...
  return (
    <div
      className="jrn-scroll"
      style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "24px 32px 32px", maxWidth: 760 }}
    >
                    ...
                    {children.map((child) => {
                      ...
                      return (
                        <NestedTaskCard
                          ...
                          onOpen={() => {
                            if (childTarget) openEntity(childTarget);
                          }}
                        />
                      );
                    })}
      ...
      {!hasLiveTopics && (
        <>
          ...
          {!isLoading && (historicalProposals ?? []).length === 0 && (
            <div style={{ color: "var(--skin-ink-faint)", fontSize: 14 }}>
...
function HistoricalProposalCard({
  ...
  const body =
    (proposal.payload.body_markdown as string | undefined) ??
    (proposal.payload.description as string | undefined);
  ...
  const typeLabel =
    proposal.proposal_type === "new_objective"
      ? "Objective"
      : proposal.proposal_type === "add_note"
        ? (proposal.payload.note_type as string | undefined) === "task"
          ? "Task"
          : "Note"
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          <span
            style={{ ...badgeBase, background: "var(--skin-surface2)", color: "var(--skin-ink)" }}
          >
            {typeLabel}
          </span>
          {isPending && (
      ...
      {objectiveTitle && (
        <p style={{ fontSize: 12, color: "var(--skin-ink-faint)", margin: "0 0 10px" }}>
          → {objectiveTitle}
        </p>
      )}
