...
export function getTTSError(): TTSErrorInfo | null {
  return lastError;
}
...
async function runJob(job: Job): Promise<void> {
  ...
  if (cachedPromise) {
    ...
    if (cachedBlob) {
      ...
      job.opts?.onStatus?.(
        finalReason === "cancelled" ? "stopped" : finalReason === "error" ? "error" : "idle",
      );
  ...
  if (res.status === 204) {
    emitError({
      ...
      status: 204,
      message:
        "Text-to-speech is not configured. Set ELEVENLABS_API_KEY in Supabase and redeploy the `elevenlabs-tts` edge function.",
    });
    if (currentJob === job) {
      disposeCurrent();
      currentJob = null;
